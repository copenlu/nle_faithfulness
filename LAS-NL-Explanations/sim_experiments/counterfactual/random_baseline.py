...
def eval_examples(args, device, examples, model, tokenizer, sample_exps):
    ...
    preds_list = []
    for step, batch in tqdm(enumerate(dataloader), total=len(examples)//args.dev_batch_size):
        # unpack batch vars
...
def eval_dataset(examples, tokenizer, args):
    with torch.no_grad():
        total_success, total_label_success = defaultdict(lambda: 0), defaultdict(lambda: 0)

        in_filled_examples = []
        for example_orig in tqdm(examples):
            example_perturbed = []
            if data_name == 'NLI':
                if random.randint(0, 1):
                    masked_examples = random_mask(example_orig.hypothesis, n_positions=args.n_pos, n_random=args.num_return_sequences)
                    for masked_text, insertion in masked_examples:
                        example = copy.deepcopy(example_orig)
                        example.hypothesis = masked_text
                        example.replacement = insertion
                        example_perturbed.append(example)
                else:
                    masked_examples = random_mask(example_orig.premise, n_positions=args.n_pos, n_random=args.num_return_sequences)
                    for masked_text, insertion in masked_examples:
                        example = copy.deepcopy(example_orig)
                        example.premise = masked_text
                        example.replacement = insertion
                        example_perturbed.append(example)
            elif data_name == 'QA':
                masked_examples = random_mask(example_orig.question, n_positions=args.n_pos, n_random=args.num_return_sequences)
                for masked_text, insertion in masked_examples:
                    example = copy.deepcopy(example_orig)
                    example.question = masked_text
                    example.replacement = insertion
                    example_perturbed.append(example)
            elif data_name == 'COMVE':
                if random.randint(0, 1):
                    masked_examples = random_mask(example_orig.sent1, n_positions=args.n_pos, n_random=args.num_return_sequences)
                    for masked_text, insertion in masked_examples:
                        example = copy.deepcopy(example_orig)
                        example.sent1 = masked_text
                        example.replacement = insertion
                        example_perturbed.append(example)
                else:
                    masked_examples = random_mask(example_orig.sent2, n_positions=args.n_pos, n_random=args.num_return_sequences)
                    for masked_text, insertion in masked_examples:
                        example = copy.deepcopy(example_orig)
                        example.sent2 = masked_text
                        example.replacement = insertion
                        example_perturbed.append(example)
            # print(len(example_perturbed))

            in_filled_examples += example_perturbed
        ...
        for i in range(len(in_filled_examples)):
            if int(in_filled_examples[i].label) != int(teacher_predictions[i]):
                print(f'{data_name}_{prefinetuned_name}_random_examples.jsonl')
                with open(f'{data_name}_{prefinetuned_name}_random_examples.jsonl', 'a') as outfile:

                    total_label_success[in_filled_examples[i].idx] += 1
                    inserted_tokens = [in_filled_examples[i].replacement.lower()]
            ...
            #         else:
            #             print('non adv', vars(in_filled_examples[i]),teacher_predictions[i], teacher_explanations[i])
            # else:
            #     print('non counter', vars(in_filled_examples[i]), teacher_predictions[i], teacher_explanations[i])
