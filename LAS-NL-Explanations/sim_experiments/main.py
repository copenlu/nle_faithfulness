...
from models.LongT5ForMC import LongT5ModelForMC
from models.BARTForMC import BARTModelForMC
...
def load_data(args, data_name, tokenizer):
    ...
    if data_name == 'QA':
        ...
        read_function = FC_data_utils.read_PubHealth
        if 't5' in args.task_pretrained_name:
            prep_function = FC_data_utils.get_tensors_for_T5_split
        elif 'bart' in args.task_pretrained_name:
            prep_function = FC_data_utils.get_tensors_for_T5_split
        elif 'bert' in args.task_pretrained_name:
            prep_function = FC_data_utils.get_tensors_for_bert
        extension = 'tsv'
...
def load_model(args, device, tokenizer, multi_gpu = True, finetuned_path = None):
    ...
    elif 'long-t5' in args.task_pretrained_name:
        model_class = LongT5ModelForMC
        model = model_class.from_pretrained(args.task_pretrained_name,
            project_to_small=False,
            cache_dir=args.cache_dir)
    elif 't5' in args.task_pretrained_name:
...
def train_or_eval_epoch(args, device, dataloader, stats_dict, multi_gpu,
                ...
                sample_exps, split_name, write_predictions = False, eval_rouge = True):
    ...
    for step, batch in enumerate(dataloader):
        ...
        if args.input_dropout > 0 and allow_dropout:
            ...
            if 't5' in args.task_pretrained_name or 'bart' in args.task_pretrained_name:
                if 'v1.0' in args.data_dir or 'ECQA' in args.data_dir:
                    x_start_idx = len(tokenizer.encode('task: [CLS]'))
                elif 'comve' in args.data_dir:
                    x_start_idx = len(tokenizer.encode('qa sentence 1: [CLS]'))
                else:
                    x_start_idx = len(tokenizer.encode('nli premise: [CLS]'))
            elif 'bert' in args.task_pretrained_name:
                x_start_idx = 1
                x_lens = x_lens + 2
            dropout_idx = np.random.choice(eligible_idx, replace=False, size = num_to_dropout)
        ...
        with grad_req:
            ...
            if args.do_task:                
                if ('t5' in args.task_pretrained_name or 'bart' in args.task_pretrained_name) and not ST_RA:
                    outputs = model(input_ids = task_input_ids, 
                                attention_mask = task_input_masks, return_dict=False)
                ...
                elif ('t5' in args.task_pretrained_name or 'bart' in args.task_pretrained_name) and ST_RA:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dev_batch_size", default=10, type=int, help="Total batch size for eval.")
    ...
    parser.add_argument('--stain_threshold',
                        help='Flag for staining the dataset.', default=100, type=int)
    ...
    if '1.0' in args.data_dir:
        data_name = 'QA'
    elif 'ECQA' in args.data_dir:
        data_name = 'QA'
        args.max_seq_length = 300
        args.max_sample_len = 300
    elif 'PUBHEALTH' in args.data_dir or 'fc' in args.model_name:
        data_name = 'FC'
        if args.hypothesis_only:
            args.max_seq_length = 200
            args.max_sample_len = 200
        else:
            args.max_seq_length = 500
            args.max_sample_len = 500
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.task_pretrained_name)
    ...
    if args.do_eval:
        ...
        stats_dict = train_or_eval_epoch(args, device, dev_dataloader, stats_dict, multi_gpu = multi_gpu,
                               ...
                               split_name = 'dev')
        if ('v1.0' not in args.data_dir) or args.labels_to_use != 'label':
        ...
        if ('v1.0' not in args.data_dir) or args.labels_to_use != 'label': # CQA does not have test labels. so compute test on other datasets and CQA simulation
            if args.do_task:
